...
func (r *Resolver) ContentTypeService() crud.Service[model.ContentType] {
	if r.ContentTypeSvc == nil {
		panic("ContentTypeService is not configured on Resolver")
	}
	return r.ContentTypeSvc
}

func (r *Resolver) GetContentType(ctx context.Context, id string) (*model.ContentType, error) {
	if err := r.guard(ctx, "ContentType", "show"); err != nil {
		return nil, err
	}
	svc := r.ContentTypeService()
	record, err := svc.Show(r.crudContext(ctx), id, nil)
	if err != nil {
		return nil, err
	}
...
func (r *Resolver) ListContentType(ctx context.Context, pagination *model.PaginationInput, orderBy []*model.OrderByInput, filter []*model.FilterInput) (*model.ContentTypeConnection, error) {
	if err := r.guard(ctx, "ContentType", "index"); err != nil {
		return nil, err
	}
	criteria := buildCriteria("ContentType", pagination, orderBy, filter)
	svc := r.ContentTypeService()
	records, total, err := svc.Index(r.crudContext(ctx), criteria)
	if err != nil {
		return nil, err
	...
	limit, offset := paginationBounds(pagination, len(records))
	result := make([]*model.ContentType, 0, len(records))
	for i := range records {
		result = append(result, &records[i])
	}
	edges := make([]*model.ContentTypeEdge, 0, len(result))
	for i := range result {
...
func (r *Resolver) CreateContentType(ctx context.Context, input model.CreateContentTypeInput) (*model.ContentType, error) {
	if err := r.guard(ctx, "ContentType", "create"); err != nil {
		return nil, err
	}
	var record model.ContentType
	applyInput(&record, input)
	svc := r.ContentTypeService()
	record, err := svc.Create(r.crudContext(ctx), record)
	if err != nil {
...
func (r *Resolver) UpdateContentType(ctx context.Context, id string, input model.UpdateContentTypeInput) (*model.ContentType, error) {
...
func (r *Resolver) DeleteContentType(ctx context.Context, id string) (bool, error) {
